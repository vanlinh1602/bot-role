...
export const handler = async (client: Client, message: Message) => {
  ...
  const roles = await Roles.findOne({ where: { guildId: message.guildId } });

  // Get the user or member's permission level from the elevation
  const level = await permLevel(message, roles || undefined);
  ...
  if (level < LevelCache[cmd.conf.permLevel]) {
    if (DEFAULT_SETTINGS.systemNotice) {
  ...
  try {
    await cmd.run(client, message, args, level, { flags });
    Logger.debug(
