...
export const handler = async (client: Client, interaction: BaseInteraction) => {
  ...
  const roles = await Roles.findOne({ where: { guildId: interaction.guildId } });
  // Get the user or member's permission level from the elevation
  const level = await permLevel(interaction, roles || undefined);
  ...
  if (level < LevelCache[cmd.conf.permLevel]) {
    ...
    await interaction.reply({
      ...
      ephemeral: DEFAULT_SETTINGS.systemNotice,
    });
    return;
  }
